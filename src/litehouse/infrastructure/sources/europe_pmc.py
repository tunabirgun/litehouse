...
class EuropePmcConnector:
    ...
    ENDPOINT_URL = "https://www.ebi.ac.uk/europepmc/webservices/rest/search"

    def __init__(self, fetcher: OfficialSourceFetcher | None = None) -> None:
        self._fetcher = fetcher or OfficialSourceFetcher()

    def build_search_request(self, search_term: str, *, page_size: int = 25) -> FetchRequest:
    ...
    async def search(self, search_term: str, *, page_size: int = 25) -> FetchResult:
        request = self.build_search_request(search_term, page_size=page_size)
